...
def format_table(html_data):
    def repl_table(m):
        # 竖着合并的表格先不拆
        if re.search('rowspan="\d+"', m.group()): return m.group()
        ...
        for row in rows:
            ...
            if len(cells) >= 3 and len(rows) in [4, 5]: break
            for cell in cells:
        ...
        if flag:
            ...
            for p in p_tags:
                if not p.get_text().strip():
                    p.extract()
            #清除表格格式
            cleaned_html = re.sub('</?(table|tr|td|tbody)[^>]*>', '', str(soup))
            cleaned_html = re.sub('class="align-center"|text-align:center', '', cleaned_html)
            #清除缩进
            cleaned_html = re.sub(' style=";text-indent:[\d\.]+pt"', '', cleaned_html)
...
def split_long_table(html_data):
    ...
    def repl_table(m):
        ...
        for idx, row in enumerate(table_soup.find_all('tr')):
            ...
            if FLAG_split == "split_table":
                mode_split = True
                list_rows.append("###SPLIT###" + "###LINES###" + row_data + "###SPLIT###")
    ...
    html_data = re.sub(r'<table[^<>]*?>((?!</table|<table)[\s\S])*?</table>', repl_table, html_data)
