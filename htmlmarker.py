    ...
    def remove_watermarks(self, html_content, pattern_key=None, file_name=None):
        ...
        if pattern_key is None:
            for key, regex in self.regex_patterns.items():
                ...
                watermark_elements = soup.find_all(
                    string=re.compile(regex, re.IGNORECASE))
                img_elements = soup.find_all(
                    'img', alt=re.compile(regex, re.IGNORECASE))
                ...
                for p_element in p_elements:
                    ...
                    combined_text = ''.join(span.get_text()
                    ...
                    if regex and re.search(regex, combined_text, re.IGNORECASE):
                        ...
                        for span in span_elements:
                            ...
                            print(f"已清除水印: {combined_text}, replaced with {
                                  span.string}")
            ...
            if regex:
                watermark_elements = soup.find_all(
                    text=re.compile(regex, re.IGNORECASE))
                img_elements = soup.find_all(
                    'img', alt=re.compile(regex, re.IGNORECASE))
                p_elements = soup.find_all('p')
        ...
        for filename in os.listdir(self.input_folder):
            ...
            if filename.endswith(".html"):
                ...
                output_filepath = os.path.join(
                    self.output_folder, f"processed_{filename}")

                with open(input_filepath, 'r', encoding='utf-8') as file:
                    html_content = file.read()

                # use marker
                cleaned_html = self.marker.remove_watermarks(
                    html_content, pattern_key=pattern_key, file_name=filename)
